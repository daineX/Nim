from logging import debug, error, info, warn, addHandler, newConsoleLogger
...
proc newTransport*(host: cstring, port: cint, vhost: cstring, user: cstring,
                   password: cstring, exchange: cstring = "domain-events",
                   exchange_type: cstring = "topic"): Transport =
    var
        conn = connect(host, port, vhost, user, password)
        channel: Channel = 1
    discard channel_open(conn, channel)
    exchange_declare(conn, channel, exchange, exchange_type)
    result = Transport(conn: conn,
                       current_channel: channel,
                       exchange: exchange,
...
proc exchange_declare*(transport: Transport, exchange: cstring, durable: bool = false, arguments: Arguments = empty_arguments) =
    exchange_declare(transport.conn, transport.channel, exchange,
                     transport.exchange_type, durable=durable,
                     arguments=arguments)

proc queue_declare*(transport: Transport, queue_name: string, passive: bool = false,
                    durable: bool = false, exclusive: bool = false,
                    auto_delete: bool = false, arguments: Arguments = empty_arguments) =
    discard queue_declare(transport.conn, transport.channel, cstring_bytes(queue_name),
                          Boolean(passive), Boolean(durable), Boolean(exclusive),
                          Boolean(auto_delete), arguments)

proc newDomainEvent*(data: JsonNode, routing_key: string = "", domain_object_id: int = 0,
                     uuid_string: string = "", timestamp: float = 0.0,
                     retries: uint = 0): DomainEvent =
    DomainEvent(routing_key: routing_key,
                data: data,
...
proc newDomainEventFromJson*(json_msg: string): DomainEvent =
    let
        ...
        data = jsonNode["data"]
    newDomainEvent(routing_key=jsonNode{"routing_key"}.getStr(),
                   data=data,
...
proc bind_routing_keys(transport: Transport, exchange: cstring, queue_name: cstring,
                       binding_keys: seq[string]) =
    for binding_key in binding_keys:
        queue_bind(transport.conn, transport.channel, cstring_bytes(queue_name),
                   cstring_bytes(exchange), cstring_bytes(binding_key), empty_arguments)
        transport.check

proc register*(transport: Transport, callback: consume_callback, name: string,
               binding_keys: seq[string], dead_letter: bool = false, durable: bool = true,
               exclusive: bool = false, auto_delete: bool = false, max_retries: uint = 0) =
    let
    ...
    transport.queue_declare(name, durable=durable, exclusive=exclusive,
    ...
    transport.check
    discard basic_consume(transport.conn, channel, cstring_bytes(name),
                          cstring_bytes(name), AMQP_FALSE, AMQP_FALSE, AMQP_FALSE, empty_arguments)
    transport.check
...
when isMainModule:
    ...
    try:
        ...
        proc always_retry(event: DomainEvent) =
            raise Retry(delay: 2.pow(float(event.retries) + 1.0))
        ...
        let err = CheckError(getCurrentException())
        error err.msg
        quit 1
